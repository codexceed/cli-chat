...
- Tool calls in a single turn run concurrently via `ToolExecutor.execute_batch` under one consolidated spinner
- Async stdin via `loop.add_reader` (not `asyncio.to_thread(input)`) — avoids dangling threads
- History stays LLM-valid on cancel: pre-batch cancel stubs every call with `[cancelled by user]`; mid-batch cancel preserves completed results and marks in-flight ones with a cancelled `ToolResult`
